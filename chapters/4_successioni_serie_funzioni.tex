...
\definition
Sia $A\subseteq \R$ e $\brackets{f_n:n\in\N}$ una successione di funzioni definite su $A$ a valori in $ \R$. Sia $B\subseteq A$
\begin{description}
	\item[-] la successione $\brackets{f_n:n\N}$ è puntualmente convergente su $B$ se $\forall x \in B$, esiste finito il limite $\lim\limits_{n\to\infty}f_n(x)$. In tal caso la funzione $f:B\to  \R$ definito da $f(x)=\lim\limits_{n\to\infty}f_n(x)$
...
$\lim\limits_{n\to\infty}f_n(x)=\lim\limits_{n\to\infty}\frac{1}{n}sin(x)=0\quad\forall x \in A \implies f_n \pconvarrow 0$ su $A$.
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
\[\lim\limits_{n\to\infty}x^{n} = \brackets{\begin{matrix}+\infty&&x\in\left]1,+\infty\right[\\ 1 && x\in\left\{1}\\0&&x\in\left]-1,1\right[\\\nexists &&x\in\left]-\infty,-1\right] \end{matrix}\right.\]
...
\begin{proof}
	Dire che $f_n$ è debolmente crescente su $B$ significa che \[\forall x_1,x_2\in B \quad x_1 \leq x_2 \implies fn(x_1) \leq f_n(x_2)\]
	se si fa tendere $n\to\infty$ si ha che $f(x_1) \leq f(x_2)$, e questo nell'insieme $B$ dove i limiti puntuali delle funzioni esistono per ipotesi
\end{proof}
\observation
...
Sia $f_n:A\to \R$, $B\subseteq A$, $A\subseteq \R$, $f:B\to A$\\
Se $f_n \pconvarrow f$ su $B$ e se $f_n$ è non negativa, $f_n \geq 0$, su $B$ allora il limite $f$ è non negativo, $f \geq 0$, su $B$.
\begin{proof}
...
\proposition
Siano $A\subseteq \R, B\subseteq A$, $\brackets{f_n:n\in\N}$ una successione di funzioni definite su $A$ con valori in $ \R$. sia $f:B\to \R$ una funzione\\
$f_n \pconvarrow f$ su $B$ per $n\to\infty \iff \forall x\in B,\forall \epsilon >0,\exists\nu\in\N:\forall n>\nu \quad \abs{f_n(x)-f(x)} \leq \epsilon$
\begin{proof}
	direttamene dalla definizione...
\end{proof}
\proposition
Siano $A\subseteq \R, B\subseteq A$, $\brackets{f_n:n\in\N}$ una successione di funzioni definite su $A$ con valori in $ \R$. Sia $F:B\to \R$ una funzione\\
$\sum\limits_{n=0}^{\infty}f_n \pconvarrow F$ su $B$ per $n\to\infty \iff \forall x\in B,\forall \epsilon >0,\exists\nu\in\N:\forall n>\nu \quad \abs{\sum\limits_{k=0}^{\infty}f_k(x)-F(x)} \leq \epsilon$
\begin{proof}
	direttamene dalla definizione...
\end{proof}
...
In $\cntclass{0}(\left[0,1\right]; \R)$ con $d_\infty(f,g)=\sup_{\left[0,1\right]}\abs{g(x)-f(x)}$, prendiamo l'insieme $C=\overline{B(0,1)}=\brackets{f\in \cntclass{0}(\left[0,1\right]; \R):\abs{f(x)} \leq 1, \forall x \in\left[0,1\right]}$
\begin{description}
...
\definition
Date due successioni di numeri reali $\brackets{a_n:n\in\N}$,$\brackets{b_n:n\in\N\setminus\left\{0}\right\}$, si defininisce serie trigonometrica di coefficienti  $\brackets{a_n:n\in\N}$,$\brackets{b_n:n\in\N\setminus\left\{0}\right\}$ la serie
