...
\begin{observation}
	...
	\begin{align*}
		...
		x_n &= s_n - s_{n - 1}
	\end{align*}
	Quindi ogni definizione o proposizione enunciata in relazione alle successioni ha un \textit{alter ego} relativa alle serie e viceversa. Non sempre per entrambe le versioni di una definizione o proposizione sono equivalentemente interessanti. Vedasi \fullref{def:AAAAAAAAA}.
\end{observation}
\begin{observation}
...
\subsection{L'Insufficienza della Convergenza Puntuale}
...
\begin{definition}[Successione Uniformemente Convergente]
	...
	\begin{note}
		...
		In questo caso invece la $f$ è una funzione \textit{tale per cui} la distanza massima (per via del $\sup$ di $d_{\cntclass{0}}$) tra $f_n$ e $f$ è $0$. Questo vuol dire che la funzione "coincide con la $f_n$ su tutto $B$", in quel senso \textit{Uniformemente}.
	\end{note}
...
\begin{proposition}
	...
	Siano $B \subseteq A \subseteq \R$ e $\brackets{f_n:\; n \to \N}$ una \textbf{Successione di Funzioni} e $f_n:\; A \to \R$. Sia $f:\; B \to \R$ una funzione:
	\[
...
\begin{corollary}
	...
	\begin{proof}
		Da \fullref{prop:conv_unif_lim_succ}  si sa che valgono condizioni analoghe, ma più restrittive, rispetto a quelle di \fullref{prop:conv_punt_lim_succ}, dunque sicuramente è valida l'implicazione $\impliedby$ di \fullref{prop:conv_punt_lim_succ}.
	\end{proof}
\end{corollary}
\begin{proposition}
	\label{prop:dist_lin_non_cont}
	% TODO Proposizione 19.34
\end{proposition}
\begin{example}
	\label{ex:BBBBBBBBBB}
...
\begin{definition}
	\label{def:AAAAAAAAA}
	% TODO Definizione 19.42 dal libro
\end{definition}
...
\begin{proposition}
	...
	\begin{proof}
		...
		$\implies\lim\limits_{n\to\infty}d_{\cntclass{0}}(fn,f)=0\quad \implies f_n\to f\in \cntclass{0}(A; \R)$
	\end{proof}
\end{proposition}
