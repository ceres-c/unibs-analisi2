...
\begin{example}[Equazione di Eulero]
	Nella meccanica orbitale, l'\textbf{Equazione di Eulero} è un importante risultato che mette in relazione varie proprietà dell'orbita di un oggetto soggetto ad una forza centrale fornendo la legge oraria del suo moto. Dati
	\begin{itemize}[noitemsep]
		...
		\item \makebox[2ex]{$\tau$\hfill} \textit{Istante} di tempo di passaggio per il pericentro
	\end{itemize}
	Si ha la
	\[\underbrace{E - e \sin(E)}_{= M} = n (t - \tau)\]
	Dalla quale è però impossibile esplicitare il tempo $t$, ed è dimostrabile che non si possa nemmeno trovare la forma esatta di una soluzione di $t$.\\
	È possibile, al massimo, avvicinarsi alla soluzione con metodi di approssimazione.
\end{example}
\begin{definition}[Funzione Implicita]
	...
	Cioè la Funzione Implicita associa una variabile dell'equazione alle altre variabili. Da questo deriva il nome \textit{implicita}, in quanto la $f$ definisce "implicitamente" la $\varphi$, perchè l'immagine di $f$ è influenzata da tutte le variabili della funzione, che dunque son legate tra loro.
	\begin{note}
...
\begin{example}
	...
	\begin{equation}
		...
		\funcdef{\varphi_1}{\intervalopen{-1}{1}}{\intervalopen{0}{1}}{x}{\sqrt{1-x^2}}
	\end{equation}
	Un primo problema nasce dalla scelta degli intervalli $\mathcal{X}$ e $\mathcal{Y}$, per esempio posso scegliere $\mathcal{X} = \intervalopen{-\frac{1}{2}}{\frac{1}{2}}$ e $\mathcal{Y} = R^+ $
	Un secondo problema è la scelta del punto nel cui intorno si cerca la funzione implicita. Sarebbe stato possibile scegliere il punto $(\frac{1}{\sqrt{2}},\frac{1}{\sqrt{2}})$ oppure ancora $(-\frac{1}{2},-\frac{\sqrt{3}}{2})$. Sicuramente $(1,0)$ non è un punto valido, dato che non si verifica il punto 3 della \fullref{def:funz_impl}, perché una eventuale relazione come $\varphi_3$ da \autoref{fig:ese_funz_impl_cerchio} non sarebbe una funzione.
	\begin{note}
	...
	~
	\begin{figure}[H]
		\caption{Funzioni implicite della $f$}
		...
		\begin{tikzpicture}
			...
			\draw[line width=.5mm, red, opacity=.5] (axis cs: .94, .34) arc [radius=100, start angle=20, end angle=160];
			...
			\draw[line width=.5mm, blue, opacity=.5] (axis cs: .97, -.26) arc [radius=100, start angle=-15, end angle=15];
			\addlegendimage{line width=.5mm, opacity=.5, color=blue}
