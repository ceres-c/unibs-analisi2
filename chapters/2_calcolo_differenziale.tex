...
\section{Funzioni a Valori in \texorpdfstring{$\protect\mathbb{C}$}{C}}
\label{sec:fun_in_C}
Sia $A \subseteq \R^n$ non vuoto. Ogni funzione $A \mapsto \mathbb{C}$ può essere identificata in modo naturale con una funzione $A \mapsto \R^2$ e viceversa.
\begin{exercise}
	\label{ex:ext_def_in_C}
	Utilizzando questa identificazione, estendere al caso di funzioni $A \mapsto \mathbb{C}$ le definizioni di continuità, derivabilità, differenziabilità e degli spazi $\cntclass{k}$ date per funzioni $A \mapsto \R$
	% TODO solution
\end{exercise}
%TODO This section is a placeholder