...
\begin{example}
	...
	La funzione del seguente grafico è derivabile in $(0,0)$ solo lungo $y$ perché non presenta discontinuità. Scegliendo una qualsiasi altra direzione non risulta continua e, dunque, derivabile.
	\begin{figure}[H]
...
\begin{theorem}[del Differenziale Totale]
	...
	\begin{proof}
		...
		\end{itemize}
		Ci si muove lungo le direzioni parallele agli assi per andare da $(x_0 + h, y_0 + k)$ a $(x_0, y_0)$
		\begin{center}
			\begin{tikzpicture}
		...
		\[
			\exists \alpha, \beta \in \realintervalopen{0}{1} \textbf{ per cui} \qquad
			\begin{array}{rrcl}
		...
		\begin{note}
			...
			\[c \in \realintervalopen{a}{b} : f'(c) = \frac{f(b)-f(a)}{b - a}\]
			Alternativamente, nella forma
			\[\xi \in \realintervalopen{0}{1} : f'(x_0 + \xi h) = \frac{f(x_0 + h)-f(x_0)}{h}\]
		\end{note}
	\end{proof}
\end{theorem}
\begin{exercise}
...
\begin{definition}[Segmento]
	...
	\[S = \brackets{tx_1+(1-t)x_0 \in \R^n:\; t \in \realintervalclose{0}{1}}\]
\end{definition}
\begin{observation}
...
\begin{exercise}
	...
	\begin{solution}
		Posta
		\[\funcdef{\varphi}{\realintervalclose{0}{1}}{\R^n}{t}{t x_1 + (1-t) x_0}\]
		\begin{itemize}
			\item $\realintervalclose{0}{1} \subset \R^n$, essendo sottoinsieme di $\R^n$ chiuso e limitato, grazie alla \fullref{prop:compat_chius_lim_Rn}, è compatto. Inoltre, grazie al \fullref{teo:weier_generale}, $\varphi(\realintervalclose{0}{1})$ è sicuramente compatto a sua volta, dunque il segmento $S$ è compatto.
...
\begin{exercise}
	...
	\begin{solution}
		\[s = \brackets{x_0+(x_1-x_0)t \in \R^n: t \in \realintervalclop{0}{+\infty} \;}\]
	\end{solution}
\end{exercise}
\begin{theorem}[degli Accrescimenti Finiti]
	...
	\begin{proof}
		...
		\begin{equation}
			...
			\funcdef{F}{\realintervalclose{0}{1}}{\R}{t}{v \cdot f \bigl( t x_1 + (1-t)x_0 \bigr)}
		\end{equation}
		\begin{note}
		...
		\end{note}
		$F$ è continua e derivabile sull'intervallo di definizione $\realintervalclose{0}{1}$. È dunque possibile utilizzare il Teorema di Lagrange da Analisi 1, che garantisce:
		\[\exists c \in \realintervalopen{0}{1}: \quad F(1) - F(0) = F'(c) \; (1-0)\]
		...
		Da cui
		\[\norm{f(x_1) - f(x_0)} \leq \norm{Df\bigl( cx_1 + (1-c)x_0 \bigr)} \cdot \norm{x_1 - x_0}\]
		e passando al $\sup$ su $t \in \realintervalopen{0}{1}$ si ottiene la tesi.
	\end{proof}
\end{theorem}
...
\begin{definition}[Insieme Convesso]
	...
	\begin{equation*}
		\begin{gathered}
			...
			\bydef\\
			\forall x_0,y_1 \in C \quad \brackets{tx_1+(1-t)x_0:\; t \in \realintervalclose{0}{1}} \subseteq C
		\end{gathered}
	\end{equation*}
...
\begin{exercise}
	...
	\begin{solution}
		...
		\[\exists t \in \realintervalclose{0}{1}:\; \forall x_1 \in S_1 \text{ e } \forall x_2 \in S_2 \quad tx_1 + (1-t)x_2 \notin S_1 \text{ e contemporaneamente } \notin S_2\]
...
\begin{definition}[Funzione Convessa]
	...
	\begin{equation*}
		...
		\quad \bydef \quad
		\begin{cases}
			\forall x_0, x_1 \in I,\; \forall t \in \realintervalclose{0}{1}\\
