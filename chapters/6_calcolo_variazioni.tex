...
\begin{proof}
	...
	\[\forall i=1,2,\ldots,n \text{ scelgo } v_j(x)=\left\{\begin{matrix}0&&j\ne i\\v_i(x)&&j=i \end{matrix}\right.\]
...
\begin{proof}
	È sempre lo stesso lemma con l'aggiunta che la funzione $v$ sia di classe $\cntclass{k}$.
	\begin{center}
	...
	Dunque le ipotesi del \fullref{lemma:fond_calc_var} sono soddisfatte ed è dimostrato.
\end{proof}
\observation
...
\begin{theorem}[Equazione di Eulero]
	...
	Quest'ultima equazione è detta equazione di Eulero-Lagrange del Funzionale $F$ o a volte detta variazione prima del funzionale $F$.
	È un sistema di $n$ equazioni differenziali ordinarie del secondo ordine nella funzione incognita $x_\ast$.\\
	...
	\begin{proof}
		...
		$\varphi$ è derivabile rispetto a $h$, dunque, se si considera la derivata direzionale lungo
		$ \vec{v} = \begin{bmatrix}
			v \\
			\dot{v}
		\end{bmatrix} $,
		...
		Ricordando che $\varphi(0)$ è un punto singolare, e dunque $\varphi'(0) = 0$, sono dunque soddisfatte le ipotesi del \fullref{lemma:fond_calc_var} e si ha
...
Si prenda ora come \textbf{caso di studio il \textcolor{red}{piano}} (anziché la sfera o altre figure). Si fissi l'origine in uno dei due punti, sia $A(x_A,y_A)$ l'altro. Posto il funzionale
\[
	X = \brackets{
...
La lunghezza della curva definita in \fullref{prop:lunghezza_curva_integrale}, data la definizione di $X$ in questo caso di studio diviene
...
e la sostituzione
\[ \psi(x') = \sqrt{1 + (x')^2} \]
l'\reffull{teo:equazione_eulero} corrispondente moltiplicando a destra e sinistra per $x'$
\begin{align*}
...
\begin{note}
	Nei prossimi passaggi verranno usate diverse relazioni trigonometriche facilmente dimostrabili. Se non note, fare riferimento alla \fullref{sect:for_trigo}.
\end{note}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[scale=2.5,cap=round,>=latex]
		\draw[->] (-1.5cm,0cm) -- (4cm,0cm) node[right,fill=white] {$x$};

		% circonferenza a sinistra
		\draw[smooth] (0cm,0.4cm) circle(0.4cm);
		\filldraw[black] (0cm,0cm) circle(0.4pt) node[anchor=north]{P};
		\draw[smooth] (0cm,0.4cm) -- +(270:0.4cm);

		% circonferenza in centro
		\draw[smooth] (pi*0.4cm,0.4cm) circle(0.4cm);
		\filldraw[black] (pi*0.4cm,0.8cm) circle(0.4pt) node[anchor=south]{P};
		\draw[smooth] (pi*0.4cm,0.4cm) -- +(90:0.4cm);

		% circonferenza a destra
		\draw[smooth] (2*pi*0.4cm,0.4cm) circle(0.4cm);
		\filldraw[black] (2*pi*0.4cm,0cm) circle(0.4pt) node[anchor=north]{P};
		\draw[smooth] (2*pi*0.4cm,0.4cm) -- +(270:0.4cm);

		% non usare smooth. La funzione diviene appunto smooth e si perdono le cuspidi
		\draw[domain=-1*pi:3*pi,very thick,blue,samples=100,variable=\t] plot ({0.4*(\t - sin(deg(\t)))}, {0.4*(1 - cos(deg(\t)))});
	\end{tikzpicture}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[scale=3,cap=round,>=latex]
		...
		\coordinate (P) at (3cm,-1cm);

		\filldraw[black] (O) circle(0.4pt) node[anchor=south]{$(0,0)$};
		\filldraw[black] (P) circle(0.4pt) node[anchor=west]{$(\overline{x},\overline{y})$};

		\draw[smooth,blue,thin] (O) -- (P);
		\draw[smooth,blue,thin] (O) -- (0cm,-1cm);
		\draw[smooth,blue,thin] (0cm,-1cm) -- (P);

		% non usare smooth. La funzione diviene appunto smooth e si perdono le cuspidi
		\draw[domain=0:1.288*pi,very thick,red,samples=20,variable=\t] plot ({0.621*(\t - sin(deg(\t)))}, {-0.621*(1 - cos(deg(\t)))});
	\end{tikzpicture}
