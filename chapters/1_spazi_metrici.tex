...
\begin{example}[Esempi di Metriche]
	...
	\begin{enumerate}
		...
		\item $\begin{array}{c}
			...
			\quad d_\infty(f,g) = \sup\limits_{x \in \intervalclose{a}{b}} \abs{g(x) - f(x)}
		\end{array}$
		\hfill
			{\footnotesize
				\begin{tabular}{c}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item \textit{Individuazione di una Successione di Cauchy}\\
				...
				\end{equation}
				La  successione $x:\N \to X$ è sicuramente ad elementi in $X$ in quanto $T$, contrazione, è per definizione $T: X \to X$. È necessario dimostrare che la $x$ sia di Cauchy (\fullref{def:cond_cau}) cioè che:
				\[\forall \varepsilon > 0\quad \exists \nu \in \N:\; \forall n,m > \nu \text{ vale } d(x_n, x_m) < \varepsilon\]
...
In tutto questo paragrafo si hanno $n \in \N$ e $n \geq 1$. Con $\R^n$ viene indicato lo spazio metrico $\R$ dotato dell'usuale \textbf{Metrica Euclidea}: $d(x,y) = \sqrt{\sum\limits_{i = 1}^{k} (y_i - x_i)^2}$ da \fullref{ex:metriche}. Questo spazio si dice anche \fullref{def:sp_vett_normato}.
\begin{note}
...
\begin{proposition}
	...
	\begin{equation*}
		...
		\begin{cases}
			...
			\end{array}
			% \mathmakebox[7em][l]{\lim\limits_{x \to x_0} (f + g)(x)}
			% \quad \exists \text{ finito e} \quad
			% \lim\limits_{x \to x_0} (f + g)(x) = \lim\limits_{x \to x_0} f(x) + \lim\limits_{x \to x_0} g(x)\\
			% \mathmakebox[7em][l]{\lim\limits_{x \to x_0} (f \cdot g)(x)}
			% \quad \exists \text{ finito e} \quad
			% \lim\limits_{x \to x_0} (f \cdot g)(x) = \lim\limits_{x \to x_0} f(x) \cdot \lim\limits_{x \to x_0} g(x)
		\end{cases}
	\end{equation*}
...
\begin{theorem}[del Confronto]
	...
	Allora:
	\begin{enumerate}
		\item \label{itm:teo_confronto_tesi_1} \hfil % To get the equation centered
			$L_f < L_g \quad \implies \quad \exists r > 0:\; \forall x \in A \text{ con } d(x, x_0) < r \text{ e } x \neq x_0 \quad f(x) < g(x)$
		\item  \label{itm:teo_confronto_tesi_2} \hfil
