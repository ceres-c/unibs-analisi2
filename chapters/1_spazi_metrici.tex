...
\begin{proposition}
	...
	\begin{proof}
		Un punto isolato, per definizione, appartiene ad $A$, e rientra quindi automaticamente nella definizione di $\overline{A}$. Altrettanto, un punto \(x_* \in X\) di accumulazione per \(A\) rientra nella definizione di \(\overline{A}\).
	\end{proof}
\end{proposition}
\begin{proposition}
...
\begin{definition}[Limite per Successioni] % TODO la definizione di limite per successioni va modificata per offrire la classica distanza delta-epsilon
	...
	\begin{equation}
		\label{eq:lim_succ}
		\lim\limits_{n \to +\infty} x_n = x_\infty \quad \bydef \quad \lim\limits_{n \to +\infty} d(x_n,x_\infty) = 0
	\end{equation}
	Se $\lim\limits_{n \to +\infty} x_n = x_\infty$, allora la successione $x_n$ è \textbf{Convergente} a $x_\infty$. Si può scrivere: $x_n \to x_\infty$ per $n \to \infty$.
