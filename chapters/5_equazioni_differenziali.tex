...
\begin{example}[Un Problema di Cauchy senza soluzione]
	Sia
	$$\funcdef{f}{\R}{\R}{x}{\begin{cases} +1 \quad \text{se}\; x < 0\\ -1 \quad \text{se}\; x \geq 0 \end{cases}}$$
...
\begin{example}[Un Problema di Cauchy con infinite soluzioni]
	Vedere \fullref{ex:pennello_peano}
\end{example}
\begin{exercise}
			...
			\resizebox{\linewidth}{!}{
				\begin{tikzpicture}
					\begin{axis} [
						...
						\addplot [domain=-3:-.2,samples=10] {exp(ln(abs(x))/3)};
						\addplot [domain=-.2:.2,samples=100,smooth] {exp(ln(abs(x))/3)};
						\addplot [domain=.2:3,samples=10] {exp(ln(abs(x))/3)};
					\end{axis}
				\end{tikzpicture}
			}
			\caption{No Loc. lips.\newline No lips.}
			...
			\resizebox{\linewidth}{!}{
				\begin{tikzpicture}
					\begin{axis} [
						...
						\addplot[domain=-.2:-.005,samples=1000]{sin(deg(1/x))};
						\addplot[domain=.005: 0.2,samples=1000]{sin(deg(1/x))};
		...
		\begin{subfigure}{.5\textwidth}
			...
			\resizebox{\linewidth}{!}{
				\begin{tikzpicture}
					...
					\addplot [domain=-10:-.5, samples=50, smooth] {(abs(x))^(1/2)};
					\addplot [domain=-.5:.5, samples=200, smooth] {(abs(x))^(1/2)};
					\addplot [domain=.5:10, samples=50, smooth] {(abs(x))^(1/2)};
					\end{axis}
				\end{tikzpicture}
			}
		\end{subfigure}
		\begin{subfigure}{.5\textwidth}
			...
			\resizebox{\linewidth}{!}{
				\begin{tikzpicture}
					...
					\addplot [domain=-7:0, samples=50, smooth] {-1/4*x^2};
					\addplot [domain=0:7, samples=50, smooth] {1/4*x^2};
					\end{axis}
				\end{tikzpicture}
			}
		\end{subfigure}
