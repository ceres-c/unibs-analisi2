...
\begin{proof}
	sia $x\in A \Leftarrow \exists B(x_0,r)\subseteq A$ con $r>0$, anche $\overline{B(x_0,r)}\subseteq A$ scelto $r$ abbastanza piccolo, allora $\norm{f(t,x_2)-f(t,x_1)}le\sup\limits_{(t,x)\in I\times \overline{B(x_0,r)}}\norm{Df()}\norm{x_2-x_1}  $ per la formula degli accrescimenti finiti.  
\end{proof}
...
$$\frac{ x'}{\sqrt{\abs{x}}}=1 \Rightarrow \int\limits_{0}^{t}{\frac{ x'}{\sqrt{\abs{x}}}dt} = t$$
$$\int\limits_{x(0)=0}^{x(t)}{\frac{1}{\sqrt{\abs{x}}}dx} = t$$
