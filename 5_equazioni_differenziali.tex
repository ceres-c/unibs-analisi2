...
\begin{proof}(\textbf{Tesi 2})
	\begin{note}
		L'idea alla base della dimostrazione è che vogliamo riuscire a trasformare il problema di Cauchy in un problema di punto fisso.
	\end{note}
	\noindent Prima di tutto serve una relazione che, dal problema di Cauchy, ci permetta di ottenere $x$ in funzione di qualcosa che dipenda da $x$. % ?????
	...
	V è massimo di funzione continua (per ipotesi 2) in un compatto (per ipotesi 1, essendo in $\R^n\times\R^n$ e per \ref{prop:int_compatto}). % TODO in cap. 1, prop 2.35: compatto <=> chiuso+limtato (intervallo)
	Indica il massimo di $f$ una volta che sia stata limitata nell'insieme di partenza\\ 
	Definiamo dunque un generico $\delta>0$
	$$\delta<\brackets{\delta_1,\frac{\rho}{V},\frac{1}{L}}$$
	\begin{itemize}
		\item $\delta_1$ è raggio di un generico intervallo incluso in $I$ di partenza
		\item $\frac{\rho}{V}$ è rapporto tra il raggio di $\overline{B(x_0,\rho)}$, palla interamente contenuta in $A$, e $V$, valore massimo di $f$ ridotta all'intervallo di cui sopra e alla palla $\overline{B}$. Questo rapporto rappresenta il reciproco del massimo di $f$ "rispetto al raggio" di $\overline{B}$, cioè una "metrica" di quanto sia piccolo $\rho$ rispetto a $\sup f$ limitata.
	...
	$(X,d_X)$ è spazio metrico completo se considerato con la distanza della convergenza uniforme $d_X$ per la \ref{prop:compl_dist_conv}. Trovato questo spazio, possiamo passare al punto chiave della dimostrazione.\\
