...
\begin{proof}(\textbf{Tesi 2})
	...
	\begin{itemize}
		...
		$(X,d_X)$ è spazio metrico completo se considerato con la distanza della convergenza uniforme $d_X$ per la \ref{prop:compl_dist_spm_compl}.\qed
		...
		\begin{proof}
		\begin{align*}
			\noaligneq{$\bigl(T(x_2)\bigr)(t) - \bigl(T(x_2)\bigr)(t) =$}
			\intertext{per definizione della $T$ e per la proprietà addittiva degli integrali}
			&= \int_{t_0}^t \Bigl(
				f\bigl(\tau,x_2(\tau)\bigr) - f\bigl(\tau,x_2(\tau)\bigr)
				\Bigr)\integrald{\tau}\\
		\end{align*}
		Dunque, passando alla norma
		\begin{align}
			\norm{\bigl(T(x_2)\bigr)(t) - \bigl(T(x_2)\bigr)(t)} \label{equaz:cau_loc_verif_contraz_norm}
		\end{align}
		È sicuramente minorata da
		\begin{align*}
			&\leq \abs{\int_{t_0}^t
				\norm{f\bigl(\tau,x_2(\tau)\bigr) - f\bigl(\tau,x_2(\tau)\bigr)}
				\integrald{\tau}}\\
			\intertext{Per definizione di loc. lips. \ref{def:loc_lips} passo alle soluzioni $x_i$ di $f$, minorando}
			&\leq \abs{\int_{t_0}^t
				L\cdot\norm{x_2(\tau)-x_1(\tau)}
				\integrald{\tau}}\\
			\intertext{Dalla \ref{obs:dist_conv_unif}, $\norm{f}_{\cntclass{0}} = \sup_A \abs{f} \implies \norm{f-g}_{\cntclass{0}} = \sup_A \abs{f-g} \leq d_{\cntclass{0}}(f,g)$}
			% TODO qui sarebbe giusto scrivere direttamente: Dalla \ref{obs:dist_conv_unif}, $\norm{f-g}_{\cntclass{0}} = \sup_A \abs{f-g} = d_{\cntclass{0}}(f,g)$
			&\leq \abs{\int_{t_0}^t % TODO se il TODO sopra è vero, qui va = al posto di <=
				L\cdot d_X(x_2,x_1)
				\integrald{\tau}}\\
			&\leq L \cdot \abs{t-t_0} \cdot d_X(x_2,x_1) \tageq\label{equaz:cau_loc_verif_contraz_fine} % TODO qui non va = al posto di <= ?
		\end{align*}
...
\begin{exercise}
	...
	% Deriva dal sup (abs(x)) a pagina 4
\end{exercise}
