...
Il problema di Cauchy è quindi $\left\{\begin{matrix}\dot{x}=k\cdot x\\x(0)=0\end{matrix}\right.$ con $x\in \R, k>0 $ e $ x_0=0$

\begin{center}
	\begin{tikzpicture}[scale=1] %[x={10.0pt},y={10.0pt}]
	\pgfmathsetmacro\MAX{2}
	\draw[->] (-\MAX,0) -- (\MAX,0) node[anchor=north west] {x};
	\draw[->] (0,-\MAX) -- (0,\MAX) node[anchor=south east] {$\dot{x}$};
	\draw[domain=-2:2,smooth,variable=\x] plot ({\x},{\x}) node {$\dot{x}=kx$};
	\draw node at (0.5,0) {$|$};
	\draw node at (1,0) {$|$};
	\draw node at (1.5,0) {$|$};
	\draw node at (0,0.5) {$-$};
	...
	\draw node[anchor=east] at (0,0.5) {$\dot{x}^{'} $};
	\draw node[anchor=east] at (0,1) {$\dot{x}^{''} $};
	\draw node[anchor=east] at (0,1.5) {$\dot{x}^{'''}$};
...
\definition Una funzione $f:I\times A\to \R^{?}$ .... $A=A\circdot{A}\subseteq \R^n$, si dice localmente lipschitziana??? .... uniformemente rispetto a $t$ se $$\forall X_0 \in A, \exists ???? : \forall x_1,x_2 \in B(x_0,r)\cap A, \forall t\in I, .......$$
\proposition
...
Si consideri il seguente problema di Cauchy: $\left\{\begin{matrix}\dot{x}=f(t,x)\\x(t_0)=x_0\end{matrix}\right.$ con $f:I\times A\in \R^n$ soddisfacente alle ipotesi:
\begin{enumerate}
...
$\left\{\begin{matrix}\dot{x}=\sqrt{|x|}\\x(0)=x_0\end{matrix}\right.$
\begin{center}
	\begin{tikzpicture}[scale=1] %[x={10.0pt},y={10.0pt}]
	\pgfmathsetmacro\MAX{2}
	\draw[->] (-\MAX,0) -- (\MAX,0) node[anchor=north west] {x};
...
$x(t)=0$ soddisfa la condizione iniziale ma ovviamente non può essere soluzione del problema poiché per $x\ne 0$ si ha che $\dot{x}=\pm 1$ che non è la derivata della funzione nulla.\\
partendo sempre dalla condizione iniziale si può ipotizzare per esempio che la soluzione cresca, solo che questo contraddice $\dot{x}(0)=-1$\\
...
\begin{proof}
	...
	$\left\{\begin{matrix}\dot{x}=f(t,x(t))\\x(t_M)=x_{t_M}\end{matrix}\right.$. Possiamo applicare quanto trovato per il punto uno della tesi, cioè esiste un intervallo contenente $t_M$ nella sua parte interna in cui la soluzione esiste ed è unica.\\
...
\begin{enumerate}
	...
	\item $f$ è sublineare
\end{enumerate}
allora il problema di Cauchy $\left\{\begin{matrix}\dot{x}=f(t,x)\\x(t_0)=x_0\end{matrix}\right.$ ammette un'unica soluzione definita sull'intervallo $I$.
\begin{proof}
	DISEGNO...\\
