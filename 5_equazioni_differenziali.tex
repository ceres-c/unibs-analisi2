...
\begin{definition}
	...
	vale che
	$$\Vert f(t,x_2)-f(t,x_1)\Vert\leq L\cdot\Vert x_2-x_1\Vert\qquad o, ugualmente\qquad\frac{\Vert f(t,x_2)-f(t,x_1)\Vert}{\Vert x_2-x_1\Vert}\leq L$$
...
\end{definition}
\note
La località è data da $x_1,x_2 \in (B(x_0,r)\cap A)$ e l'uniformità da $\forall t\in I$

\proposition
Siano $I\subseteq \R$ un intervallo aperto ed $A\subseteq \R^n$ un aperto. Ogni funzione $f\in \cntclass{1}(I\times A,R^n)$ è localmente lipschitziana??? ..... uniformemente rispetto a $t\in I$.
\begin{proof}
...
Si consideri il seguente problema di Cauchy: $\left\{\begin{matrix} x'=f(t,x)\\x(t_0)=x_0\end{matrix}\right.$ con $f:I\times A\in \R^n$ soddisfacente alle ipotesi:
\begin{enumerate}
	\item $I\subseteq \R$ intervallo, $A\subseteq \R^n, t_0\in \circdot{I}, x_0\in \circdot{A}$
...
\begin{description}
	...
	\item[-] $t_0 \in \circdot{J}, \varphi(t_0)=x_0$
	\item[-] $\varphi$ derivabile e $\varphi'(t)=f(t,\varphi(t))$
\end{description}
\observation 
?????????????????????????????????????????????\\
ESEMPIO Il Baffo/Pennello di Peano\\
$\left\{\begin{matrix} x'=\sqrt{|x|}\\x(0)=x_0\end{matrix}\right.$
\begin{center}
	\begin{tikzpicture}[scale=1] %[x={10.0pt},y={10.0pt}]
	\pgfmathsetmacro\MAX{2}
	\draw[->] (-\MAX,0) -- (\MAX,0) node[anchor=north west] {x};
...
\begin{enumerate}
	...
	\le 
	(\left\| x_0-y_0 \right\|+\delta\left\| f-g \right\|_{\cntclass{0}})e^{L|t-t_0|}  
	$$
	dove $\left\| f-g \right\|_{\cntclass{0}}=\sup\limits_{I\times A}\left\|f(t,x)-g(t,x)\right\|$
\end{enumerate}
\observation
...
\begin{proof}
	...
	\begin{description}
		...
		$$\left\| Tx_2 - Tx_1\right\|_{\cntclass{0}}\le K\left\|x_2-x_1\right\|_{\cntclass{0}}\quad k\in\left[0,1\right[\quad\forall t\in\left[t_0-\delta,t_0+\delta\right]$$
		...
		$$L\left| \int_{t_0}^t \left\|x_2-x_1\right\|_{\cntclass{0}(\left[t_0-\delta,t_0+\delta\right];R^n)} d\tau \right|=$$
		$$L\cdot \left\| x_2-x_1 \right\|_{\cntclass{0}(\left[t_0-\delta,t_0+\delta\right];R^n)}\left|t-t_0\right|\le$$
		$$L\delta\left\|x-x_0\right\|_{\cntclass{0}(\left[t_0-\delta,t_0+\delta\right];R^n)}$$
		cioè $\forall t \in \left[t_0-\delta,t_0+\delta\right]$ vale che $\left\|(Tx_2)(t)-(Tx_1)(t)\right\|\le L\delta\left\|x_1-x_2\right\|_{\cntclass{0}(\left[t_0-\delta,t_0+\delta\right];R^n)}$ e poiché è vero $\forall t \Rightarrow $ passando all'estremo superiore
		$$ \left\| Tx_1 - Tx_2 \right\|_{\cntclass{0}(\left[t_0-\delta,t_0+\delta\right];R^n)}\le L\delta\left\|x_1-x_2\right\|_{\cntclass{0}(\left[t_0-\delta,t_0+\delta\right];R^n)}$$
	...
	$$ \left\| \varphi(t)-\psi(t)\right\|\le\left[\left\|x_0-y_0\right\|+\delta\left\|f-g\right\|_{\cntclass{0}}\right]+e^{L\left|t-t_0\right|}$$
