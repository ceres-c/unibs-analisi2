...
\observation
la covergenza uniforme equivale alla convergenza rispetto alla distanza $d_{\cntclass{0}}(f,g)=\sup\limits_{x\in A}\left|g(x)-f(x)\right|$ ogniqualvolta questa distanza è definita.\\
...
%Il limite puntuale di questa successione di funzioni è $f\left( x \right)=\left|x\right|$ su $A$\\
%Si può verificare che $f\left(x\right)=\left|x\right|$ è anche limite uniforme su $A$ poichè
%$$ \lim\limits_{n\to\infty}\sup\limits_{x\in A} \left|\sqrt{x^2+\frac{1}{n}}-\left|x\right|\right| $$
...
\end{proof}
\proposition
Sia $A$ un compatto in $ \R$. In $\cntclass{0}(A; \R)$ sia $d_{\cntclass{0}}=\sup_A\left| g(x)-f(x) \right|$. Allora $\left( \cntclass{0}\left(A; \R\right);d_{\cntclass{0}}\right)$ è uno spazio metrico completo.
\begin{proof}
...
In $\cntclass{0}(\left[0,1\right]; \R)$ con $d_\infty(f,g)=\sup_{\left[0,1\right]}\left|g(x)-f(x)\right|$, prendiamo l'insieme $C=\overline{B(0,1)}=\left\{f\in \cntclass{0}(\left[0,1\right]; \R):\left|f(x)\right|\le 1, \forall x \in\left[0,1\right]\right\}$
\begin{description}
...
QUI GRAFICO ..............\\
..........................\\
Questa serieconverge esclusivamente per $\left|x\right|<1$, mentre la funzione $f$ è definita su tutto $ \R$, $f\in \cntclass{\infty}( \R; \R)$\\
...
\end{enumerate}
\definition
Il prodotto scalare in $\cntclass{0} \rightleftharpoons \left<f,g\right>=\int_{-\pi}^{\pi}f(x)g(x)dx$
