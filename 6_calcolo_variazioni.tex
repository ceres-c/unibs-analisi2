...
x & \to & \int_{a}^b f(t,x(t),\dot{x}(t))dt\end{array}$$
...
$$\sum\limits_{i=1}^{N}\left\| \dot{\gamma}(t_i)(t_i-t_{i-1})\right\| =??$$
$$\int_{a}^{b}\left\| \dot{\gamma}(t) \right\| dt$$
\proposition
Se $\gamma\in C^1(\realintervalclose{a}{b};\mathbb{R}^n)\Rightarrow l(\gamma)=\int_a^b\left\|\dot{\gamma}(t)\right\|dt$
\observation
...
$$\begin{array}{ccc} 
F: X & \to & \mathbb{R} \\
x & \to & \int_{a}^b \left\|\dot{x}(t)\right\|dt
\end{array}$$
$$\begin{array}{ccc} 
f: \left[a,b\right]\times\mathbb{R}^2\times\mathbb{R}^2 & \to & \mathbb{R} \\
(t,x,\dot{x}) & \to &  \left\|\dot{x}(t)\right\|
\end{array}$$
...
\begin{proof}
	...
	Allora $\exists a,b\in\left]0,1\right[$ t.c. $x_0\in\left]a,b\right[ e \forall x\in\left]a,b\right[$ vale che $\left|f(x)\right|\ge\left|f(x_0)\right|$ sempre per la continuità di $f$.\\
...
\begin{proof}
	...
	Se $v$ è un po più regolare , prendiamo $v=u^(k+1)(x)$. cioè se vogliamo $v\in C^k$ prendiamo.....\\
...
Allora $\partial_xf(t,x_\ast(t),\dot{x_\ast}(t))-\frac{d}{dt}\partial_{\dot{x}}f(t,x_\ast(t),\dot{x_\ast}(t))=0$.\\
